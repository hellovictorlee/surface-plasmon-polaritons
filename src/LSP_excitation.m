...
for j=1:2*r+1
    ...
    for i=1:2*r+1
        ...
        eparray(j+ycen-r,i+xcen-r) = 11.575*ep;
...
wide_of_needle=5;
r_p(y_position-wide_of_needle:y_position+wide_of_needle,50:nx)     = r_p_value;
omega_p(y_position-wide_of_needle:y_position+wide_of_needle,50:nx) = omega_p_value;
eparray(y_position-wide_of_needle:y_position+wide_of_needle,50:nx) = ep_*ep;
%}
...
for t=1:5000
    ...
    if mod(t,5)==0
        %{
        subplot(2,2,1)
        plot(Hzi(y_position-wide_of_needle-1,:));
        axis([0 nx -2 2]);
        %axis tight;
        title(['t = ', num2str(t)]);
        ...
        subplot(2,2,3)
        plot(Hz(y_position-wide_of_needle-1,:));
        axis([0 nx -2 2]);
        %axis tight;
        title(['t = ', num2str(t)]);
...
%writegif('Hz_circle_plane.gif',frame,0.05);
